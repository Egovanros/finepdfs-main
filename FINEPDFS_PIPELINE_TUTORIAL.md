...
Вариант с **Docling** (результаты в `./finepdfs/data_docling/`, не перезаписывает OpenDataLoader):

```powershell
powershell -ExecutionPolicy Bypass -File run_docling.ps1 --crawl-ids CC-MAIN-2023-06
```
...
## Сводная таблица метрик по шагам (учебный прогон)

Ниже — **одна сводная таблица** по уже выполненным прогонам на этом репозитории. **Пайплайн заново не запускался**; цифры взяты из артефактов на диске и логов DataTrove (`logs/2026-05-27_*`).

**Условия прогона**

| Параметр | Значение |
|----------|----------|
| `LIMIT` (строк индекса CC на краул) | `5000` |
| `--crawl-ids` | `CC-MAIN-2024-10` |
| `--gpus` | `0` (RolmOCR и Qwen на шагах 3–4 пропущены) |
| OCR-роутинг | `ocr_prob ≥ 0.2` или `garbled_text_ratio > 0` |
| `OCR_PREDICTOR_NUM_PAGES_TO_SAMPLE` | `8` |
| GlotLID `language_threshold` | `0.01` |
| Таймаут извлечения PDF | `600` с |
| Шаги 1–2 | один общий прогон (до ветвления экстрактора) |
| Шаги 3–5 | два отдельных скрипта: `run_finepdfs_pipeline.py` (OpenDataLoader → `./finepdfs/data/`) и `run_finepdfs_pipeline_docling.py` (Docling → `./finepdfs/data_docling/`) |

**Сводная таблица**

| Шаг | Метрика | Общее (шаги 1–2) | OpenDataLoader | Docling |
|-----|---------|------------------|----------------|--------|
| **1** Фильтр и загрузка PDF | Полные PDF (не обрезанные, из WARC) | **36** | — | — |
| 1 | Обрезанные PDF (догрузка по HTTP) | **3** | — | — |
| 1 | Ошибка загрузки PDF | **1** | — | — |
| **2** Дедупликация и OCR-роутинг | Полные PDF: дубликаты / на OCR / без OCR / ошибка OCR | **0 / 5 / 21 / 10** | — | — |
| 2 | Обрезанные PDF: дубликаты / на OCR / без OCR / ошибка OCR | **0 / 1 / 3 / 0** | — | — |
| 2 | Всего PDF без OCR (вход шага 3) | **24** | — | — |
| **3** Извлечение текста (только без OCR, CPU) | Полные PDF: извлечено / ошибка | — | **21 / 0** | **21 / 0** |
| 3 | Обрезанные PDF: извлечено / ошибка | — | **2 / 1** | **3 / 0** |
| 3 | Всего извлечено | — | **23** | **24** |
| 3 | Доля успеха от входа «без OCR» (24 PDF) | — | **95,8%** | **100%** |
| 3 | Время извлечения (бенчмарк, 24 PDF) | — | **47,6 с** (~**2,27** с/док.) | **130,5 с** (~**6,21** с/док.); Docling ~**2,7×** медленнее |
| **4** Постобработка (без Qwen при `--gpus 0`) | Документов после постобработки | — | **23** | **24** |
| 4 | Отброшено пустых документов | — | **0** | **0** |
| **5** Языковой фильтр (GlotLID) | Документов после разбивки по языку | — | **23** | **24** |
| 5 | Токены Llama (сумма / среднее на документ) | — | **55 089 / 2 395** | **60 720 / 2 530** |
| 5 | Язык | — | eng_Latn - 12 (52,2%)<br>jpn_Jpan - 2 (8,7%)<br>spa_Latn - 2 (8,7%)<br>anp_Deva - 1 (4,3%)<br>bos_Latn - 1 (4,3%)<br>por_Latn - 1 (4,3%)<br>zxx_Latn - 1 (4,3%)<br>unknown - 1 (4,3%)<br>eng_Latn_removed - 1 (4,3%)<br>ita_Latn_removed - 1 (4,3%) | eng_Latn - 13 (54,2%)<br>ita_Latn - 2 (8,3%)<br>jpn_Jpan - 2 (8,3%)<br>spa_Latn - 2 (8,3%)<br>bos_Latn - 1 (4,2%)<br>por_Latn - 1 (4,2%)<br>zxx_Latn - 1 (4,2%)<br>hun_Latn_removed - 1 (4,2%)<br>eng_Latn_removed - 1 (4,2%) |
| **6** Точная дедупликация по тексту | Документов на выходе | — | *не замерялось в A/B* | *не замерялось в A/B* |
| **7** Классификация EDU / DCLM | Метки EDU и DCLM | — | *не выполнялся в A/B* | *не выполнялся в A/B* |
| **8** MinHash (почти-дедуп) | Кластеры / итоговый выход | — | *не выполнялся в A/B* | *не выполнялся в A/B* |

**Примечания**

- Колонки **OpenDataLoader** и **Docling** для шагов **3–5** сопоставимы: один и тот же вход после шага 2 (**24** PDF без OCR); для сравнения запускайте `run.ps1` и `run_docling.ps1` (или два `python`-скрипта) и сравнивайте `./finepdfs/data/` с `./finepdfs/data_docling/`.
- Среди **обрезанных** PDF OpenDataLoader не извлёк **1** документ (CLI exit 1); Docling извлёк все **3** — отсюда разница **23 vs 24** на шагах 3–5. Проценты по языкам (шаг 5) — от **23** и **24** документов соответственно.
- Полный прогон `run.ps1` / `python run_finepdfs_pipeline.py` на этом ПК на **шаге 7** (`run_model_classification`) останавливался с **401 Unauthorized** на `mmbert-colab/mmBERT-base` (нужен доступ Hugging Face). Шаги **6–8** в таблице A/B не гонялись.
- Источники: шаги 1–2 — общий прогон; OpenDataLoader — `logs/2026-05-27_09-50-29_vnvrs` (3–4), `logs/2026-05-27_09-50-44_veafz` (5); Docling — `logs/2026-05-27_09-54-50_smulx` (4), `logs/2026-05-27_09-55-04_mcpou` (5).
